...
def train(q, q_target, mix_net, mix_net_target, memory, optimizer, gamma, batch_size, update_iter=10, chunk_size=10, grad_clip_norm=5):
...
def main(exp_name, lr, gamma, batch_size, buffer_limit, log_interval, checkpoint, max_episodes, max_epsilon, min_epsilon,
    ...
    writer = SummaryWriter(log_dir=os.path.join("qmix_experiments/runs", exp_name, time.strftime("%Y-%m-%d_%H-%M-%S")))
    # create env.
    env = MultiAgentEnv(agents=10, k=6, range_start=[0,50])
    test_env = MultiAgentEnv(agents=10, k=6, range_start=[0,50])
    memory = ReplayBuffer(buffer_limit, chunk_size=10, n_agents=env.num_particles, input_shape=[env.k])
    ...
    mix_net = MixNet(env.observation_space, recurrent=recurrent).cuda()
    mix_net_target = MixNet(env.observation_space, recurrent=recurrent).cuda()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp-name', required=False, default='QMIX_Per_dist_align_cohesion')
    parser.add_argument('--seed', type=int, default=1, required=False)
    parser.add_argument('--no-recurrent', action='store_true')
    parser.add_argument('--max-episodes', type=int, default=150000, required=False)
    parser.add_argument('--max-epsilon', type=float, default=0.95, required=False)
    ...
    parser.add_argument('--warm-up-steps', type=int, default=10000, required=False)
    ...
    kwargs = {'exp_name': args.exp_name,
              ...
              'log_interval': 1,
              'checkpoint': 'qmix_experiments/checkpoints',
